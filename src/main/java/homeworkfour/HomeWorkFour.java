...
public class HomeWorkFour {
    ...
    private static boolean isTurnValid(int y, int x) {
        ...
        if (map[y][x] == EMPTY_DOT) { // если поле свободно для игры
            return true;
        }
    ...
    private static boolean isWin(char symbol) {
        for (int i = 0; i < SIZE; i++) {
            if ((map[i][0] == symbol && map[i][1] == symbol && map[i][2] == symbol && map[i][3] == symbol && map[i][4] == symbol) ||
                    (map[0][i] == symbol && map[1][i] == symbol && map[2][i] == symbol && map[3][i] == symbol && map[4][i] == symbol) ||
                    (map[0][0] == symbol && map[1][1] == symbol && map[2][2] == symbol && map[3][3] == symbol && map[4][4] == symbol) ||
                    (map[4][0] == symbol && map[3][1] == symbol && map[2][2] == symbol && map[1][3] == symbol && map[0][4] == symbol)) {
                return true;
            }
        }
        return false;
